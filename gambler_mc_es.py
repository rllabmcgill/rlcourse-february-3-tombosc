...
"""
on-policy first-visit monte carlo with action value function and exploring starts
...
def generate_episode(pi, returns, counts):
    """
    ...
    a = np.random.choice(s) + 1 # from 1 to s
...
def policy_mc_iterate(max_iter=10000, delta=10**-6):
    ...
    for i in range(max_iter):
        print "ITERATION", i
        old_q = np.copy(q)
        generate_episode(pi, returns, counts)
...
q, pi = policy_mc_iterate(max_iter=25000)
...
plt.show()
f.savefig("gambler_es_25k_it_p_03.png")
